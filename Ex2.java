...
public class Ex2 {
	...
	public static void main(String args[]) {
		...
		n.setRightChild(new BinaryTreeNode(2L));
		root.setLeftChild(n);

		n = new BinaryTreeNode(7L);
		n.setLeftChild(new BinaryTreeNode(24L));
		n.setRightChild(new BinaryTreeNode(3L));
	...
	public static void displayTree(PrintStream out, BinaryTreeNode<Long> node) {
		...
		// pre-order traversal
		/*
		 * if (node != null) { out.println(node.getData()); displayTree(out,
		 * node.getLeftChild()); displayTree(out, node.getRightChild()); }
		 */

		// in-order traversal
		/*
		 * if (node != null) { displayTree(out, node.getLeftChild());
		 * out.println(node.getData()); displayTree(out, node.getRightChild());
		 * }
		 */
